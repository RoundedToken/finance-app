...
| **`prod_smoke.py`** | **Prod-периметр Worker'а после деплоя** (QA-10, SPEC-046): healthz 200, auth-гарды (bootstrap/web/rates → 401, tg → 403, удалённый admin/references → 404) + свежесть rates через wrangler d1 (≤3 дней). Read-only, без секретов, URL из `cloud/miniapp/.env`. | `.venv/bin/python local/scripts/prod_smoke.py [--no-d1]` |
...
| `backfill_crypto_rates.py` | Однократная заливка исторических крипто-курсов (ETH/EUR) в `rates` (SPEC-026). |
