...
│   ├── process.md                — spec-driven pipeline (правило 0)
│   ├── security.md               — security-протоколы, чеклист перед push
│   ├── setup.md                  — как поднять проект с нуля
│   ├── stack.md                  — технологический стек
│   ├── roadmap.md                — план этапов и текущий статус
│   ├── post-mvp-roadmap.md       — live-статус post-MVP фаз
│   └── review-mvp-stage1.md / sync-protocol.md / adaptive-budgets-plan.md — обзор MVP, legacy-протокол, план бюджетов
...
│   │   ├── backfill_crypto_rates.py — заливка исторических крипто-курсов (ETH)
│   │   ├── prod_smoke.py         — smoke-проверка прода после деплоя
│   │   ├── init_db.py / migrate_to_d1.py — legacy до-D1-эпохи, не использовать
│   │   └── import_legacy_snapshots.py / import_ok_csv.py — разовые импорты
│   ├── launchd/                  — plist для автозапуска backup_d1.py
│   ├── migrations/, schema.sql   — legacy локальной SQLite (до ADR-011; НЕ источник правды)
│   ├── backups/                  — копии D1 (sql dump)
│   ├── screenshots/              — скриншоты UI-харнесов (gitignored)
...
│       ├── src/                  — main.tsx, routeTree.tsx, routes/, components/, api/, lib/
