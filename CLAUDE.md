...
11. **Mini App scope зафиксирован — ввод расходов + read-only подсказки.** Не расширять Mini App новыми разделами с CRUD (снапшоты/доходы/обмены). Аналитика расходов живёт **только в Web Admin** (vanilla-аналитика была снята при React-rewrite SPEC-014 и в React не воссоздана — осознанное решение, не баг; быстрая аналитика на телефоне = план post-MVP). Допустимы **read-only подсказки в момент ввода** (без CRUD): остаток бюджета по категории «осталось X €» (SPEC-020, owner-решение 2026-05-31). Любое подобное расширение — только read-only и только если помогает решению «вводить трату/сколько»; управление (CRUD лимитов и т.п.) остаётся в Admin.
