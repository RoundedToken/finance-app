...
Тотал тренд положительный (рост уровня жизни реален). Архетипы по данным: Жильё→fixed (CoV низкий); Еда/Продукты/Досуг→recurring (CoV средний); Коммуналка→seasonal/recurring (CoV средний); Техника/Одежда/Дом→lumpy (CoV высокий); Подписки/Чаевые→intermittent. Кросс-категорийный профицит vs P75 ≈ N €/год (для линзы Фазы 4). Симуляция: плоский EWMA → ратчет-ловушка; damped-Holt держит реальность (лимит «Еды» плавно снижается за 3 мес экономии, настоящие спайки помечаются корректно).
