...
- **Ревизия чужой спеки = обратная запись в её changelog** (правило из аудита 2026-07, SPC-08). Если новая спека пересматривает/отменяет часть старой (`supersedes`/`revises`/`revives` во фронтматтере новой), в **старой** спеке в тот же коммит добавляется: строка в changelog («что и кем пересмотрено») + `revised_by:`/`superseded_by:` во фронтматтер. Иначе done-спека, прочитанная одиночно, врёт как справочник. Образец двусторонней разметки: SPEC-016 ↔ SPEC-025.
...
## Лёгкий tier для микро-фич (Стадия 2)

Полный pipeline (spec → impl → двойной gate → push) обязателен для фич с **миграциями D1, новыми endpoint'ами или изменением денежной математики**. Для откатов, edit-фич и тривиальных правок — допустим **mini-spec** (Context + Goals + Acceptance, без полного 13-секционного шаблона) и упрощённый gate (один проход review). ADR-013 уже допускает `chore(...)` без pipeline — это расширяет правило на micro-features, чтобы Phase 1 не превращалась в формальность-постфактум (ср. SPEC-009→012 за сутки, SPEC-010/SPEC-018 create→done в один день).
