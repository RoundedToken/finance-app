...
1. https://console.cloud.google.com → `New Project` → имя `finances-rates`.
...
5. Положить ключ:
   ...
   mv ~/Downloads/finances-rates-*.json ~/.config/finances-gsheets/key.json
   chmod 600 ~/.config/finances-gsheets/key.json
   ```
   **Канонический путь** (зашит в скриптах): `~/.config/finances-gsheets/key.json`.
   Service Account email — что-то вида `claude-sheets@finances-rates.iam.gserviceaccount.com`
...
Web Admin использует Google OAuth для входа. Нужен Web-OAuth Client в том же GCP-проекте, что и Service Account для Sheets (`finances-rates`).
