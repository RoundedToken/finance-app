...
> 📸 **ИСТОРИЧЕСКИЙ СНИМОК на 2026-05-28.** Стадия 2 закрыта 2026-05-29: ветка
> `refactor/mvp-stage2` **смержена в main и задеплоена**, MVP финализирован.
> Утверждения ниже («не смержено → не задеплоено», чек-лист «перед деплоем»)
> описывают состояние НА МОМЕНТ ревью и сегодня не действуют. Актуальный статус —
> `docs/roadmap.md` и `docs/post-mvp-roadmap.md`.
>
...
Ветка `refactor/mvp-stage2`, 7 коммитов (A→F). ~~Не смержено в main → не задеплоено~~ → **смержено и задеплоено 2026-05-29** (см. баннер выше).
...
~~Перед деплоем: `git checkout main && git merge refactor/mvp-stage2`, затем деплой worker + оба Pages.~~ **Выполнено 2026-05-29** — merge + деплой worker и обоих Pages состоялись, поведение overdraft/fee/asOf на проде.
