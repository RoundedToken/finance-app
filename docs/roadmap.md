...
- ✅ **SPEC-031 — фикс двойного учёта импортированной зарплаты в историческом net worth** (created_at tie-break при bulk-импорте): снапшот мог сортироваться раньше события той же даты → событие учитывалось дважды; импортёры не захардкожены под порядок. Тема смежная с SPEC-024 (event-time-ordering). Выкачено на прод.
...
- **Модель двух классов** (ADR-014): запас (вёдра / net worth / накопленное в `target_currency` → EUR) → курс на сегодня (mark-to-market); поток (расход / доход / day-total / **вклад в цель → `target_currency`**, ADR-020/SPEC-025) → курс на дату операции (date-aware). Вся конверсия — на worker через `RatesIndex`, клиент не делит на курс, получает `*_eur`.
...
- Фикс-эффект: RUB-вёдра/цели больше не дают 0 при устаревшем курсе (per-quote `rateAt` вместо глобального `MAX(date)`-snapshot); EUR-эквивалент трат стал историчным. Audit: solution-architect APPROVED_WITH_NICES, senior-qa PASS_WITH_NICES (must-fix — устаревший мок `test_admin_ui`, исправлен). test_admin_ui + test_miniapp_react проверены (скриншоты: net worth, split, EUR-колонки, day-total RUB→EUR date-aware).
- ✅ **Пересмотрено SPEC-025/ADR-018→ADR-020** (2026-06-13): R3 этой спеки (mark-to-market goal balance) откатан — вклад в цель переклассифицирован из «запас» в «поток» (фиксируется по курсу на дату вклада). Псевдо-запас ломался на обменах целевых денег (RUB→USDT→EUR): прогресс цели дрожал по курсу валюты, которой уже нет, и в `free` протекал FX-рассинхрон. Шаг `target_currency → EUR` остался mark-to-market. `free` без клампа (может быть `<0` — danger в Admin). Без миграции D1. На проде (worker `7b5d6ace`).
...
## ✅ MVP + пост-MVP (выкачено)

**✅ Стадия 2 завершена — MVP финализирован (2026-05-29).** Все батчи punch-list'а A–F + SPEC-019 закрыты (ветка `refactor/mvp-stage2` смержена в `main` и задеплоена, см. `docs/review-mvp-stage1.md` § Прогресс). Финальный gate: `mvp_ready=yes`, 0 блокеров, все must-fix исходного ревью подтверждены закрытыми; 42 vitest + typecheck (worker/admin/miniapp) зелёные. **Идёт post-MVP** — на проде все `done`-спеки (кроме cancelled SPEC-009); источник истины «что выкачено» — `status` во фронтматтере `specs/SPEC-*.md`, не этот файл. Live-статус — `docs/post-mvp-roadmap.md`.

Все базовые потоки (Stage 8 + 8.5 Dashboard v2, SPEC-016 canonical conversion, SPEC-017 категории, SPEC-018 линза) закрыты. На проде также весь пост-MVP: Stage 5b sparklines (SPEC-021), бюджеты (SPEC-020/023), инвестиции (Stage 9 — SPEC-026…030), виртуализация списков (SPEC-034), статистика Mini App (SPEC-036), график прогресса цели (SPEC-037), coach-нуджи (SPEC-040, AI шаг 1). В работе сейчас ничего нет; открыты только по триггеру: AI шаг 2 (умные инсайты/чат через routine), keyset-пагинация, прочее Фазы 3. 🔻 В бэклоге: мобильный ввод дохода/снапшота (2.3, owner: не нужно), shared Zod-контракт (2.7-часть-2, низкий ROI).

### Post-MVP хвосты (из ревью Стадии 1, 2026-05-28)
- **Аналитика расходов в Mini App (Phase 2 SPEC-014).** При React-rewrite vanilla-аналитика (stats/donut/trend/drilldown) не воссоздана. Решение: scope Mini App сужен до «только ввод», аналитика — в Web Admin (CLAUDE.md правило 11 обновлено). Быстрая аналитика на телефоне — **план post-MVP**, не блокер MVP. ✅ **Частично (SPEC-035, 2026-06-26):** История получила фильтр по периоду (Месяц/Год/Всё) + навигацию стрелками + итог периода (EUR-эквивалент). ✅ **Полностью (SPEC-036, 2026-06-27, выкачено на прод):** owner разморозил scope (ADR-021) — вернули полноценный экран «📊 Статистика» в React Mini App (KPI + donut + список категорий + тренд + drill-down, read-only поверх трат, метрика `amount_eur`). Правило 11 + post-mvp non-goal обновлены: read-only аналитика **расходов** в Mini App разрешена; глубокая (доходы/снапшоты/портфель/net worth) остаётся в Admin.
...
- **Зарплата RSD за 2024 – середину 2025 не внесена.** EUR-нал историю добавили: 18 записей `2024-01-15…2025-06-15` на `eur-cash` (помесячные суммы — в D1, записи `legacy-salary-eur-*`; из публичного дока убраны — см. `docs/security.md`), стыкуются с существующей записью `2025-07-15`. RSD-часть (1-го числа) за тот же период — **ждёт суммы от Stepan'а**, если нужна. Backup перед вставкой: `local/backups/d1-pre-salary-20260526-213929.sql`.
...
> ✅ **Рендер (windowing) — закрыто SPEC-034 (2026-06-26):** `@tanstack/react-virtual` на двух реально длинных списках — Admin `/expenses` (row-level поверх TanStack Table, padding-row + `table-fixed` + sticky thead + `measureElement`) и Mini App История (day-level, virtual item = блок дня, container-scroll). В DOM только видимые строки + overscan. Полный load и клиентские фильтры/KPI сохранены; деньги-математика/контракты/Worker не тронуты (windowing-only по решению владельца). DayTotal (SPEC-033) и порядок (SPEC-024) сохранены. Прод-замер: Admin 19→35 DOM-строк из 400, Mini App 11→14 блоков-дней из 140.
>
> 🔻 **Отложено (keyset-пагинация) — отдельный будущий SPEC по триггеру.** При ~1.8k строк сеть не проблема; а все фильтры/сортировка/KPI в Admin клиентские по полному массиву → серверная пагинация без переноса фильтров на сервер бессмысленна (клиенту всё равно нужны все строки). Триггер: реальный рост данных (10×) + готовность переносить фильтры/KPI на сервер. Прочие короткие списки (доходы/обмены/снапшоты/взносы) не виртуализированы — десятки/сотни строк, выигрыша нет.

**Проблема (исходная)**: списки грузятся целиком и рендерятся в DOM полностью — Mini App `useExpenses` (`limit=20000` после SPC-06), Admin `/expenses`/`/incomes`/`/transactions`/`/snapshots` + contributions (`limit=20000`). Данные растут (>1800 трат и далее) → тяжелеет и сетевая загрузка, и рендер тысяч узлов. Текущий «lazy-рендер на клиенте» в Mini App — это отложенный рендер, **не** настоящая виртуализация. (Рендер закрыт SPEC-034; сетевая часть = keyset, отложена.)
...
- [ ] Balance over time per bucket (step chart) — **единственное реально открытое в Stage 5b**.
- [x] Net worth over time — ✅ де-факто доставлен дашбордом (SPEC-013/018: ECharts «net worth over time» с toggle форма/валюта).
...
### Stage 9 — Инвестиции ✅ (крипто-портфель, SPEC-026/027/028/029/030 — на проде)
...
- ✅ Стейкинг (stETH/Lido/Bybit): признак позиции + доход из снапшотов (ground truth) + APR-прогноз пунктиром; частичный стейкинг + авто-APR с Lido (SPEC-027); живой прогноз накопления по APR + ожидаемый €/год (SPEC-030).
...
- ✅ График позиции (SPEC-029/030): выбор периода (паритет с дашбордом) + дефолт-окно «Авто» от первой операции + адаптивная гранулярность (дневные точки для короткой истории); `PeriodPresets` — общий компонент.
- Отложено (NG SPEC-026/029): realized P&L при продаже, акции/ETF/банк-%, отдельная котировка stETH (пег), авто-interest-tx (конфликт с manual=ground-truth), крупный line-chart портфеля.
...
- [x] `auth_date` freshness не проверяется в initData. ~~OK для single-user.~~ ✅ Закрыто SPEC-045 (аудит SEC-06): TTL 24ч.
- [ ] `bot.ts` `parseExpense` принимает любой 3-5 буквенный токен как currency без проверки.
- [x] Telegram webhook без `secret_token` header. ✅ Закрыто SPEC-042 (аудит SEC-04): секрет + constant-time проверка.
...
- [x] CSP отсутствует. ✅ Закрыто SPEC-045 (аудит SEC-10): _headers Mini App с CSP/nosniff/Referrer-Policy.
...
- [x] `jwt.ts` constant-time HMAC compare. ✅ Закрыто SPEC-045 (аудит SEC-07); `alg` из header игнорируется by design (verify пересчитывает HS256 — аудит подтвердил отсутствие alg-confusion).
...
- [x] Двойной (×3) `loadRatesIndex` на полном bootstrap-пути Mini App (`listExpenses` + `getBudgetsWithProgress` + envelopes) — ✅ закрыто SPEC-038: `getBootstrapData` грузит `RatesIndex` 1× и шарит через `rates?`-параметр (3×→1×, тест prepare-spy).
...
**Из SPEC-025 (goal envelope flow) audit:**
- [ ] Legacy-цель без `target_currency`: `listGoals` конвертит в `"EUR"` (`goals.ts`), а `getGoalDetail` — в `r.currency_code` строки → для мультивалютной legacy-цели list-баланс и detail-баланс могут разойтись. Пре-существует (фича меняла только `today→row.date`); `validateGoalPayload` теперь требует `target_currency`, страдают лишь старые записи. Выровнять обе ветки → `"EUR"`.
- [ ] Дублирование формулы `free = net − targeted − invested` в `dashboard.ts` и `index.ts` (`/accounts`) — две независимые копии, потенциальный дрейф. Кандидат на общий helper (вместе с rates-aggregator тех-долгом 2.7). Single-user — не критично.
- [ ] `index.ts` `/v1/web/accounts` `free_eur<0` не покрыт unit-тестом (только `getDashboard`). Regression-тест на accounts-summary закрыл бы дрейф второй копии формулы.

**Из диагностики SPEC-041 (медианный темп, 2026-07-01):**
- [ ] **Расходы без `account_id` не участвуют в реконструкции балансов** (`dashboard.ts` — событие добавляется только `if (e.account_id)`). Уточнение 2026-07-02: **ручной ввод Mini App счёт пишет всегда** (июнь/июль — 106/106 со счётом); без счёта — только (а) 1820 записей `csv_ok_import` 2024-01…2026-05-22 (исторический импорт «Расходы ОК», импортёр не проставлял счёт — ср. memory SPEC-031 «импортёры не захардёнены»), (б) 4 записи mini_app 22–24.05.2026 (первые дни приложения, до привязки счёта) и (в) 1 запись `telegram` — `bot.ts` текстовый ввод счёт не привязывает до сих пор. На ТЕКУЩУЮ линию net worth не влияет (всё после снапшота 22.06 со счётом); влияет только на реконструкцию истории между старыми снапшотами. Действия по желанию: 5 майских записей привязать руками в Admin (мелкая сумма, уже поглощена снапшотом); `bot.ts` — дефолтное ведро по валюте; импорт — не трогать (для cashflow-аналитики счёт не нужен).
- [ ] Пунктир-проекция net worth не клампится на 0 (история — да): при отрицательном projectRate прогноз может уйти ниже нуля, пока история прижата к нулю. Пре-существует, косметика.
- [ ] Спарклайны KPI показывают ряд за выбранный период-пресет, а окно cur/prev — фикс 6 мес; цвет спарка — по окну (kpi-card-one-signal). Осознанный дизайн, зафиксировано.

**Из SPEC-034 (виртуализация / windowing) audit:**
- [ ] Живой iOS-тест swipe-delete / tap-to-edit на виртуализированных строках Mini App (`test_miniapp_ios.py`, Appium): структурно безопасно (R4 — `SwipeRow` в обычном потоке внутри блока дня, `translateX` не конфликтует с `translateY` блока), но на устройстве не гонялось. Прогнать при следующей iOS-сессии.
- [ ] Пре-существующая (не из SPEC-034) неконсистентность: поле поиска `globalFilter` на ExpensesPage сужает таблицу, но KPI («Записей»/`totalEur`/`byCurrency`) считаются по `filtered` без поиска → при активном поиске KPI не сходятся с видимой таблицей. NG2 (фильтры/KPI клиентские). Отдельный мелкий тикет: либо включить поиск в `filtered`, либо считать KPI из `table.getFilteredRowModel()`.
- [ ] Mini App: разовый «подскок» скролл-позиции у дня с bulk-импортом (день из сотен трат): `estimateSize=132` → `measureElement` пересчитывает реальную высоту при въезде в окно. Осознанно (E3/R1), наблюдать при реальном импорте.

**Из SPEC-035 (Mini App История — фильтр периода) audit:**
- [ ] Итог периода (`≈ X €`) и `DayTotal` считают `amount_eur ?? 0` → траты с пропущенным курсом даты (`amount_eur=null`, SPEC-016) тихо занижают сумму без бейджа «курс не найден». Наследие из `DayTotal`, единообразно, не регресс. При росте доли крипто/редких валют — отдельный мелкий бейдж missing-rate.
- [ ] Полный ARIA-tabs паттерн (tablist↔tabpanel): добавлены `role=tabpanel`+`aria-controls`+`aria-live`, но без `id`-привязки каждого таба к панели через `aria-labelledby`. Достаточно для текущего уровня, можно доусилить.
...
- [x] **#20**: переименовать корень с `excel/` → ✅ выполнено: корень — `finance-app/` (GitHub `RoundedToken/finance-app`); доки синхронизированы (аудит 2026-07, DOC-05).
...
**MVP финализирован (2026-05-29), идёт post-MVP.** Стадия 1 (стратегическое ревью) + Стадия 2 (рефакторинг A–F + SPEC-019) завершены и задеплоены. Math строгая (manual = ground truth, events = delta), доки синхронизированы с кодом, финмодель/метрики корректны, серверная валидация (Zod) на месте, мёртвый код вычищен. Актуальный live-статус — `docs/post-mvp-roadmap.md` (Фаза 1 закрыта, Фаза 2 в основном закрыта).
...
- Stage 5b (step-chart баланса ведра, остаток), Stage 10 (AI Coach), keyset-пагинация длинных списков (по триггеру). График прогресса цели (2.5) — ✅ SPEC-037.
- 🔻 Мобильный ввод дохода/снапшота (text-команда бота) — в бэклог (owner 2026-06-27: не нужно). Внесение RSD-зарплаты 2024–сер.2025.
- Миграция удаления спящих tx-колонок `chain_id/chain_sequence/goal_id` (~08-2026). Реальный FK через rebuild-таблицы для `expenses.account_id/category_id` (аудит 2026-07 DB-12: у `snapshots.account_id` FK есть с 0006 и энфорсится; у expenses FK нет — пока закрыто серверными exists-guard'ами DB-03, SPEC-043).
