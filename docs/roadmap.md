...
- [ ] Presence-смоук на `by_bucket_native` в `getDashboard` отсутствует (нет `dashboard.test.ts`) — контракт AC1 держится транзитивно через `reconstructBalance`-тесты, но не защищён от регрессии (уберут поле → искры молча исчезнут). Свернуть в **2.6** (D1-мок `getDashboard` + presence/equality-гард).
