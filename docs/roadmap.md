...
**Стадия 2 — глубокий рефакторинг перед закрытием MVP.** По итогам стратегического ревью (`docs/review-mvp-stage1.md`, вердикт `ready-with-caveats`) идёт чистка перед объявлением MVP завершённым. Punch-list батчами A–F в review-документе. Batch A (синхронизация доков), B (мёртвый код), C (корректность финмодели + тесты) — в работе; D/E/F — после чекпоинта.
