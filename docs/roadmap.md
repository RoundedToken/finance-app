...
Все базовые потоки (Stage 8 + 8.5 Dashboard v2, SPEC-016 canonical conversion, SPEC-017 категории, SPEC-018 линза) закрыты. На проде также весь пост-MVP: Stage 5b sparklines (SPEC-021), бюджеты (SPEC-020/023), инвестиции (Stage 9 — SPEC-026…030), виртуализация списков (SPEC-034), статистика Mini App (SPEC-036), график прогресса цели (SPEC-037). В работе сейчас ничего нет; открыты только: Stage 10 (AI Coach), мобильный ввод дохода/снапшота (2.3), keyset-пагинация (по триггеру).
