...
- **Аналитика расходов в Mini App (Phase 2 SPEC-014).** При React-rewrite vanilla-аналитика (stats/donut/trend/drilldown) не воссоздана. Решение: scope Mini App сужен до «только ввод», аналитика — в Web Admin (CLAUDE.md правило 11 обновлено). Быстрая аналитика на телефоне — **план post-MVP**, не блокер MVP. ✅ **Частично (SPEC-035, 2026-06-26):** История получила фильтр по периоду (Месяц/Год/Всё) + навигацию стрелками + итог периода (EUR-эквивалент). Это read-only навигация в рамках scope (правило 11), НЕ графики/разбивка по категориям — те остаются в Admin.
