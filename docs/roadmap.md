...
Между этапами. Stage 8 + 8.5 (Dashboard v2) и SPEC-016 (canonical conversion) закрыты. Следующее на выбор: Stage 5b (графики снапшотов), Stage 9 (инвестиции), редактирование категорий, виртуализация длинных списков.
...
- **Давний хвост: редактирование категорий** (название / эмодзи / цвет) в Admin + Mini App — ещё не сделано. Сейчас категории правятся только напрямую в D1.
...
- [ ] Residual клиентский `toEur` (latest-snapshot) в `IncomesPage` (fallback после `amount_eur`) и `DashboardPage` (goals-forecast `remainingEur`) — последний остаток «4-го варианта» конверсии вне scope SPEC-016. Добить mini-spec'ом, чтобы canonical-инвариант стал глобальным.
...
**Следующий приоритет** — date-aware rates в `/accounts` + goal balance (выровнять с dashboard `RatesIndex`), `data-model.md` update, либо Stage 5b (графики снапшотов) / Stage 9 (инвестиции).
