...
- [ ] **Расходы без `account_id` не участвуют в реконструкции балансов** (`dashboard.ts` — событие добавляется только `if (e.account_id)`; 73 расхода за май–июнь 2026 без счёта, ввод из Mini App). Между снапшотами линия net worth видит доходы, но не эти траты → «пила»: ползёт вверх, обрывается на дате очередного снапшота; точка конца месяца завышена на траты после последнего снапшота (~450 € на 30.06). Варианты: дефолтное ведро по валюте расхода / обязательный счёт в Mini App / нудж coach'а на бессчётные траты. NG5 SPEC-041, вынесено сюда осознанно.
