...
Между этапами. Stage 8 + 8.5 (Dashboard v2) закрыты. Следующее на выбор: date-aware rates в `/accounts` + goal balance (выровнять фундамент с dashboard), Stage 5b (графики снапшотов), Stage 9 (инвестиции).
...
- [ ] **Currency conversion: date-aware rate** в goal balance / `/accounts` — сейчас latest-rate. Dashboard (SPEC-013) уже считает date-aware через `RatesIndex`; распространить на остальные (общий util).
...
- [ ] AC2 edge: `/accounts` консолидирует по глобальному `MAX(date)` и даёт 0 при stale-quote; dashboard — per-quote latest (корректнее). Выровнять `/accounts` на per-quote-latest либо явно признать различие.
