...
describe("FIN-01 — смена счёта не ревальвирует сумму молча", () => {
    ...
    it("income: счёт ТОЙ ЖЕ валюты без amount → ok (валюта не меняется)", async () => {
        const { env, d1 } = makeWorld();
        seed(d1, { incomes: [{ id: "i1", date: "2026-06-07", account_id: "rub-bank", amount: 500, currency_code: "RUB" }] });
        const r = await updateIncome(env, "i1", { account_id: "rub-bank2" });
        expect(r.ok).toBe(true);
    });
    ...
    it("snapshot: перенос на ведро другой валюты без amount → отклонено; с amount → ok; та же валюта → ok", async () => {
        ...
        // ghost-ведро больше не пишется молча (parity с createSnapshot)
        expect((await updateSnapshot(env, "s1", { account_id: "ghost", amount: 1 })).ok).toBe(false);
    });
});
