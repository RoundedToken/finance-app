...
- **Note**: Worker URL захардкожен (`WORKER_BASE = "https://finances-worker.stepan-mikhalev-99.workers.dev"`) — это публичный endpoint, не секрет. Но при появлении dev/staging — extract в `wrangler.example.toml` + env-injection через build step (что противоречит «no bundler»). Принять как есть.
