...
- **AC1 — `/healthz` returns `200 {"ok":true}`** — pass.
  Live: `curl https://finances-worker.<owner>.workers.dev/healthz` → `HTTP 200 {"ok":true}` in ~40ms. Routing: `cloud/worker/src/index.ts:73`.
...
- **AC8 — Unauthenticated bootstrap returns 401** — pass.
  ...
  - Structured but bad-hash: `user={"id":<TELEGRAM_ID>,...}&auth_date=...&hash=<64-hex>` → 401
...
- **[multi-user/leak] `cloud/worker/src/db.ts:78-91`** — `listExpenses` filters by `deleted_at IS NULL` but NOT by `user_id`. In a multi-user world this returns everyone's expenses to any authorized caller. SPEC-001 §5 says "В Stage 1 всегда `user_id = '<TELEGRAM_ID>'`" — so for the current single-user MVP this is irrelevant. Note for Stage 2+: add `AND user_id = ?` once a second whitelisted user exists. (Same pattern in `getBootstrapData` at `db.ts:181-184`.)
