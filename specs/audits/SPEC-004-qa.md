...
**PASS.**
```
curl -i 'https://finances-worker.<owner>.workers.dev/v1/auth/google/start?return_to=https://finances-admin.pages.dev/'
→ HTTP 302
location: https://accounts.google.com/o/oauth2/v2/auth?client_id=<GOOGLE_CLIENT_ID>-...&redirect_uri=https%3A%2F%2Ffinances-worker.<owner>.workers.dev%2Fv1%2Fauth%2Fgoogle%2Fcallback&response_type=code&scope=openid+email&state=wIgtLEjZjG1PJfwQycKmJTgtqCayakNTg5KZqKSL-Dk&access_type=online&prompt=select_account
...
```
content-security-policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://finances-worker.<owner>.workers.dev; font-src 'self' data:; frame-ancestors 'none'
