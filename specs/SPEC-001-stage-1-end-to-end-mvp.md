...
1. Stepan открывает Telegram → находит `@finance_stepan_bot`.
...
- `user_id` — Telegram ID, в Stage 1 всегда `'45085076'` (Stepan, единственный whitelisted user).
- `deleted_at` — soft delete; в Stage 1 не задействован (DELETE добавлен в API для Stage 2).

## 6. API contract

Все endpoints на `https://finances-worker.stepan-mikhalev-99.workers.dev`. Реализация: `cloud/worker/src/index.ts`.
...
- Telegram ID `45085076` — не секрет (есть в `authorized_users`).
...
- [x] **AC1**: `curl https://finances-worker.stepan-mikhalev-99.workers.dev/healthz` → `200 {"ok":true}`.
...
- [x] **AC3**: `wrangler d1 execute finances-outbox --command="SELECT * FROM authorized_users"` содержит как минимум одну строку (`telegram_id = '45085076'`).
- [x] **AC4**: `@finance_stepan_bot` подключён к Worker. `setWebhook` → `https://finances-worker.../tg` → 200. Стартовая команда `/start` от Stepan возвращает приветствие.
...
```bash
# health
curl -s https://finances-worker.stepan-mikhalev-99.workers.dev/healthz
# → {"ok":true}

# bootstrap без auth → 401
curl -s -o - -w '%{http_code}\n' \
  https://finances-worker.stepan-mikhalev-99.workers.dev/v1/bootstrap
# → 401
