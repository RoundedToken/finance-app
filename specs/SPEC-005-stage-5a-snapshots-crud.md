...
5. Выбирает ведро (например `RUB · банк`), вводит сумму (например 250000),
...
7. Возвращается на `/accounts` — карточка `RUB · банк` показывает 250 000 RUB
...
3. Видит подсказку под суммой: «Прошлый: 250 000.00 RUB от 24.05.2026
   · +50 000.00» (если предыдущий снапшот того же ведра существует).
4. Меняет сумму на 245 000. Жмёт «Сохранить».
...
2. Браузерный `confirm()` показывает: `Удалить снапшот? RUB · банк ·
   245 000.00 · 24.05.2026`. Стёпа подтверждает.
    ...
    "accounts": [
      {
        ...
        "color": "#a78bfa",
        "is_active": 1,
        "latest_snapshot": { "id": "uuid", "date": "2026-05-24", "amount": 250000 }
      },
      ...
    ]
  }
...
│ 4 321.50 EUR         │ 5 / 7                │ 2026-05-24        │
│                      │ есть пустые          │ Источник:GFINANCE │
├──────────────────────┴──────────────────────┴───────────────────┤
│ ┌──────────┐ ┌──────────┐ ┌──────────┐                          │
│ │RUB · банк│ │RSD · банк│ │RSD · нал │ ...                      │
│ │250000RUB │ │ 12000RSD │ │  8500RSD │                          │
│ │2 600 EUR │ │  100 EUR │ │   72 EUR │                          │
...
├──────┼────────────┼────────────┼───────────────────┼────────────┤
│24.05 │🟪 RUB·банк │ 250 000 RUB│ зарплата за май   │ ✎  🗑      │
│22.05 │🟦 USDT     │     420 USDT│ —                 │ ✎  🗑      │
│ ...  │            │            │                   │            │
└──────┴────────────┴────────────┴───────────────────┴────────────┘
```
...
- Delete (`Trash2`) — `window.confirm()` с человеческой подписью
  «Удалить снапшот? RUB · банк · 250 000.00 · 24.05.2026».
...
- Подсказка под суммой (только если у выбранного ведра есть
  `latest_snapshot`): «Прошлый: 250 000.00 RUB от 24.05 · **+50 000.00**»
